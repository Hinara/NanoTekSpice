...
class Graph {
	public:
		...
		std::map<const std::string, std::unique_ptr<nts::IComponent>>	&getGraph() { return _graph; }
		std::map<const std::string, Output *>				&getOutput() { return _output; }
	...
	private:
		...
		std::map<const std::string, Output *>				_output;
